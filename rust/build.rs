...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Generate all Git metadata, including VERGEN_GIT_DESCRIBE
    let git = GixBuilder::all_git()?;
    Emitter::default().add_instructions(&git)?.emit()?;

    /* 2) Locate the C++ sources --------------------------------------- */
    let crate_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR")?);
    let repo_root = crate_dir.parent().expect("workspace root");

    // Workspace checkout → root; crates.io build → vendored copy
